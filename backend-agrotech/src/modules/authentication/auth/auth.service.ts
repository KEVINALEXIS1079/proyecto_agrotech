...
import { UsuariosService } from 'src/modules/usuario/usuarios/service/usuarios.service';
...
export class AuthService {
  ...
  async login(correo: string, contrasena: string) {
    ...
    const permisosConEstado = usuario.permisos
    ...
    return {
      ...
      usuario: {
        ...
        rol: usuario.rol?.nombre_rol,
        permisos: permisosConEstado.length > 0 ? permisosConEstado : permisosUsuario.map(p => ({ permiso: p, activo: true })),
      },
    };
  }
}