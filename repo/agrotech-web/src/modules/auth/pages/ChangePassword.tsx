// src/pages/auth_page/change-password.tsx
import {
  Input,
  Button,
  Modal,
...
import { cambiarContrasena } from "#/modules/auth/api/auth";
...
export default function ChangePassword() {
  ...
  // Del paso /code: navigate("/change-password", { state: { email, codigo } })
  const emailFromState = (location.state as { email?: string } | null)?.email;
  const codigoFromState = (location.state as { codigo?: string } | null)?.codigo;

  const email = (emailFromState || localStorage.getItem("recoveryEmail") || "").trim();
  const codigo = (codigoFromState || localStorage.getItem("recoveryCode") || "").trim();

  // Si no hay contexto, vuelve a solicitar
  useEffect(() => {
    if (!email || !codigo) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // 👇 CLAVE: usar los nombres que espera tu backend
      ...
      // Limpia rastros del flujo de recuperación
      localStorage.removeItem("recoveryEmail");
      localStorage.removeItem("recoveryCode");

      navigate("/login", { replace: true });
    } catch (e: any) {
      // Intenta mostrar mensajes del backend si vienen como array o string
      const msg =
  ...
  return (
    <div className="h-dvh overflow-hidden grid md:grid-cols-[50%_50%] bg-white">
      ...
      <div className="relative hidden md:block">
        {/* En Vite, assets de /public se referencian SIN /public */}
        ...
        <div className="w-full max-w-[420px]">
          ...
          <form className="grid gap-3" onSubmit={onSubmit}>
            <Input
              ...
              size="sm"
              value={pw}
              onChange={(e) => { setPw(e.target.value); if (showError) setShowError(false); }}
            ...
            <Input
              ...
              size="sm"
              value={cpw}
              onChange={(e) => { setCpw(e.target.value); if (showError) setShowError(false); }}
          ...
          <Modal isOpen={showError} onOpenChange={setShowError}>
            <ModalContent>
              <ModalHeader>No se pudo actualizar</ModalHeader>
              <ModalBody>
                <p>{err}</p>
              </ModalBody>
              <ModalFooter>
