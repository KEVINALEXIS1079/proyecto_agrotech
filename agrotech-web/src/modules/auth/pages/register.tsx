...
export default function RegisterPage() {
  ...
  const [goLogin, setGoLogin] = useState(false); // ← marcar si debemos ir a /login al cerrar
  ...
  async function handleSubmit(v: AuthRegisterValues) {
    ...
    (Object.keys(v) as Array<keyof typeof v>).forEach((k) => {
      if (k !== "confirmar" && k !== "acepta") fd.append(k, v[k] as string);
    });

    try {
      await mutateAsync(fd);
      setMsg("Usuario registrado con éxito");
      setGoLogin(true);   // ← éxito: al cerrar modal, navegamos
      setOpen(true);
    } catch (e: any) {
