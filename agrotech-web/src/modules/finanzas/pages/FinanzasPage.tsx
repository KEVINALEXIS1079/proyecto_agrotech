      ...
      <Card className="border border-default-200">
        <CardHeader className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <Select label="Filtrar por lote" selectedKeys={[loteFilter]} onChange={e=>setLoteFilter(e.target.value)}>
              <SelectItem key="" value="">Todos los lotes</SelectItem>
              {LOTES.map(l=><SelectItem key={l} value={l}>Lote {l}</SelectItem>)}
            </Select>
                ...
                {draftTipo && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <Select label="Nombre del tipo" selectedKeys={[draftTipo.nombre_tipo_sensor]} onChange={e=>{
                      ...
                      {(["temperatura","humedad","ph","radiacion"] as const).map(n => <SelectItem key={n} value={n}>{n}</SelectItem>)}
                    </Select>
...
function SensorForm({ draft, tipos, onChange }:{ draft: Sensor; tipos: TipoSensor[]; onChange:(s:Sensor)=>void }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      >
        {tipos.map(t => <SelectItem key={t.id_tipo_sensor_pk} value={t.id_tipo_sensor_pk} className="capitalize">{t.nombre_tipo_sensor}</SelectItem>)}
      </Select>
      <Select
        label="Lote"
