...
import SubloteMap, { type CoordenadaSublote, type SubloteExistente, type LoteExistente } from "../widgets/SubloteMap";
...
export default function EditarPageSublote() {
  ...
  const subloteId = Number(id_sublote);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (sublotes.some(s => s.nombre_sublote === nombreSublote.trim() && s.id_sublote_pk !== subloteId)) {
      setErrorNombre("El nombre del sublote ya existe.");
      return;
    }
    setErrorNombre("");

    // Validaciones de lote y coordenadas
    if (!loteSeleccionado) {
    ...
    <div className="bg-white p-8 rounded-xl shadow-md border border-gray-100 overflow-y-scroll h-[calc(100vh-10rem)] space-y-6">
      ...
      <div ref={mapContainerRef}>
        <SubloteMap
          ...
          setArea={setArea}
          sublotesExistentes={sublotesExistentes}
