...
import { Button, Chip, Input, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, Tooltip } from "@heroui/react";
import { Edit3, Plus, RefreshCcw, Trash2, RotateCcw } from "lucide-react";
...
export default function TipoSensorTable({
  ...
  const filtered = useMemo(() => {
    ...
    return (data || []).filter((r) =>
      [r.nombre_tipo_sensor, r.unidades_tipo_sensor, String(r.decimales_tipo_sensor ?? "")]
        .join(" ")
        .toLowerCase()
        .includes(txt)
    );
  ...
  return (
    <div className="space-y-4">
      ...
      <Table aria-label="Tabla de tipos de sensor" isStriped removeWrapper>
        ...
        <TableBody emptyContent={loading ? "Cargando..." : "Sin registros"} items={filtered}>
          {(row: TipoSensor) => (
            <TableRow key={row.id_tipo_sensor_pk}>
              <TableCell>
                <ImagePreview src={row.imagen_tipo_sensor || undefined} />
              </TableCell>
              <TableCell className="font-medium">{row.nombre_tipo_sensor}</TableCell>
              <TableCell>{row.unidades_tipo_sensor || <span className="text-foreground-400">—</span>}</TableCell>
              <TableCell>{row.decimales_tipo_sensor ?? <span className="text-foreground-400">—</span>}</TableCell>
                ...
                {!deleted ? (
                  ...
                  <div className="flex justify-end gap-2">
                    <Tooltip color="success" content="Restaurar">
                      <Button isIconOnly size="sm" variant="flat" color="success" onPress={() => onRestore?.(row)}>
                        <RotateCcw size={16} />
                      </Button>
                    </Tooltip>
                  </div>
