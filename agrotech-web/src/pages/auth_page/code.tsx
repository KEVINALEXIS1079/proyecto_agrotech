...
export default function Code() {
  ...
  // Email que recibiste desde /recover o que quedó persistido
  const email =
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Persistimos por si el usuario recarga o si el guard sólo mira localStorage
      localStorage.setItem("recoveryEmail", email);
      localStorage.setItem("recoveryCode", value);

      // Ir al paso final (/recovery) pasando state también
      navigate("/recovery", { state: { email, codigo: value } });
    } catch (err: any) {
