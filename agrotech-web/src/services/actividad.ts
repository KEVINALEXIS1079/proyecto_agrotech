...
export type Actividad = {
  ...
  estado_actividad: "Pendiente" | "En progreso" | "Completada"| "Cancelada";
...
export async function updateActividad(id: number, payload: Partial<Actividad>) {
...
export async function getActividadById(id: number) {
  const { data } = await api.get(`/actividades/${id}`); // /api/v1/actividades/${id}
